...
// newVerifier builds a gpg verifier trusting the test key.
func newVerifier(t *testing.T) plugin.Verifier {
	t.Helper()

	keyring, err := openpgp.ReadArmoredKeyRing(strings.NewReader(gnupg.pubKey))
	require.NoError(t, err)
	v, err := gpg.FromKeyRing(keyring)
	require.NoError(t, err)

	return v
}

// TestMatchTestBehaviourWithGit verifies that current ways of signing objects
// didn't change in newer Git versions.
func TestMatchTestBehaviourWithGit(t *testing.T) {
	...
	repo := initRepo(t)
	configureRepoSigning(t, repo, gnupg.keyID)
	...
	t.Run("commit", func(t *testing.T) {
		...
		oursRaw := injectGpgSig(unsigned, string(c.Signature))
...
func runMain(m *testing.M) int {
	...
	if !ok {
		// Tests will skip individually when gnupg is nil.
		return m.Run()
	}
...
func setupGPG() (func(), bool) {
	...
	gnupg = &gpgEnv{home: home, keyID: keyID, pubKey: pub}
...
func gpgSign(t *testing.T, payload []byte) string {
	...
	cmd := exec.Command("gpg",
		"--batch", "--pinentry-mode", "loopback", "--passphrase", "",
		"--armor", "--detach-sign", "--local-user", gnupg.keyID)
